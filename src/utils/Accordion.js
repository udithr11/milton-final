...
const Section = ({ title, description, isVisible, setIsVisible }) => {
  return (
    <div className="bg-slate-200 m-10 p-6 rounded-xl">
  <div className="flex items-center justify-between">
    ...
    {isVisible ? (
      <button onClick={() => setIsVisible(false)}>
        <img className="h-6 w-6 rounded-full " src={minus} alt="hide" />
      </button>
    ) : (
      <button onClick={() => setIsVisible(true)}>
        <img className="h-6 w-6 rounded-full " src={plus} alt="show" />
      </button>
    )}
  </div >
  {isVisible && (
